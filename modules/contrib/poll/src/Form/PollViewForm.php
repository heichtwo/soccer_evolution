...
class PollViewForm extends FormBase implements BaseFormIdInterface {
  ...
  public function showResults(PollInterface $poll, FormStateInterface $form_state) {
    // Either the "View results" button has been clicked or voting is no longer
    // allowed.
    return $form_state->get('show_results') || !$this->isVotingAllowed($poll);
  }
  ...
  protected function actions(array $form, FormStateInterface $form_state, PollInterface $poll) {
    ...
    if ($this->showResults($poll, $form_state)) {
      ...
      // Allow to go back from the results screen to the poll.
      // Poll needs to be open + you didn't vote yet, you're allowed to vote.
      if ($this->isVotingAllowed($poll)) {
        $actions['#type'] = 'actions';
  ...
  protected function isCancelAllowed(PollInterface $poll) {
    ...
    return $poll->hasUserVoted()
      ...
      && ($this->currentUser()->isAuthenticated() || !empty($_SESSION['poll_vote'][$poll->id()]))
      // And poll is open.
      && $poll->isOpen();
  }
  ...
  protected function isVotingAllowed(PollInterface $poll) {
    ...
    // If we are anonymous the poll itself needs to allow the voting of
    // anonymous users.
